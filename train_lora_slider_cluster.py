...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train AudioLDM2 LoRA slider on paired piano/guitar segments.")
    ...
    p.add_argument("--prompt_target", default=(
        "a version of this sound that preserves its exact timing, rhythm, melody, pacing, and overall event structure"
    ))
    p.add_argument("--prompt_neutral", default=(
        "a version of this sound that preserves its exact timing, rhythm, melody, pacing, and overall event structure"
    ))
    p.add_argument("--prompt_piano", default=(
        "a piano version of this sound, with piano notes matching the exact timing, rhythm, melody, pacing, and overall event structure of the original"
    ))
    p.add_argument("--prompt_guitar", default=(
        "a guitar version of this sound, with guitar notes matching the exact timing, rhythm, melody, pacing, and overall event structure of the original"
    ))

    p.add_argument("--train_steps", type=int, default=50)
    p.add_argument("--train_cfg_scale", type=float, default=3.5)
    p.add_argument("--concept_guidance_scale", type=float, default=4.0)
    p.add_argument("--target_denoise_guidance_scale", type=float, default=3.0)
    p.add_argument("--identity_loss_weight", type=float, default=1.0)
    ...
    p.add_argument("--resume_from", default=None, help="Optional path to a checkpoint to resume from.")
    p.add_argument("--train_timestep_mode", choices=["random", "middle"], default="random")
    p.add_argument("--val_timestep_mode", choices=["random", "middle"], default="middle")
    p.add_argument("--separation_loss_weight", type=float, default=1.0)
    p.add_argument("--separation_margin", type=float, default=0.0)
    p.add_argument("--beta_identity", type=float, default=0.0)
    return p.parse_args()
...
def main() -> None:
    ...
    trainer_cfg = TrainerConfig(
        ...
        target_denoise_guidance_scale=args.target_denoise_guidance_scale,
        ...
        val_timestep_mode=args.val_timestep_mode,
    ...
    if args.resume_from:
        ...
        trainer.load_training_checkpoint(args.resume_from)
